...
# 🚀 Non-Blocking Background Telegram Alert (Never Freezes Web Server)
def send_tg_alert_async(text, photo_path=None, reply_markup=None):
    def _send():
        try:
            ...
            print(f"Telegram Alert Async Error: {e}")
...
# ----------------- 🤖 TELEGRAM ADMIN BOT (FAST ATOMIC) ----------------- #
...
def handle_tg_callbacks(call):
    ...
    if data.startswith("tg_pay_appr_") or data.startswith("tg_pay_rejc_") or data.startswith("tg_appr_") or data.startswith("tg_rejc_"):
        ...
        if action == "approve":
            ...
            coins_to_give = int(pay['coins_reward'])
            if coins_to_give == 0 and "VIP" not in str(pay['item_type']):
                if float(pay['amount']) >= 1100: coins_to_give = 200
                elif float(pay['amount']) >= 600: coins_to_give = 100
                elif float(pay['amount']) >= 150: coins_to_give = 20
                else: coins_to_give = 15

            if "VIP" in str(pay['item_type']):
                exp_date = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")
                c.execute("UPDATE users SET is_vip = 1, vip_expires = ? WHERE id = ?", (exp_date, int(pay['user_id'])))
                success_text = "👑 VIP 30-Days Activated!"
            else:
                c.execute("UPDATE users SET coins = coins + ? WHERE id = ?", (coins_to_give, int(pay['user_id'])))
                success_text = f"🪙 {coins_to_give} Coins Credited!"

            txn_code = f"TXN-{random.randint(100000, 999999)}"
            c.execute("""
                INSERT INTO transactions (txn_id, user_id, username, type, amount, coins, details, admin_ref, created_at)
                VALUES (?, ?, ?, 'Payment Approved', ?, ?, ?, 'Telegram Bot', ?)
            """, (txn_code, int(pay['user_id']), str(pay['username']), float(pay['amount']), coins_to_give, f"{pay['item_type']} (#{pay['request_id']})", now_time))
...
def api_login():
    ...
    if user and check_password_hash(user['password'], password):
        if user['is_banned']:
            return jsonify({'success': False, 'msg': f"❌ Account Banned!"})
        session.permanent = True
...
def api_signup():
    ...
    try:
        ...
        conn.commit()
        send_tg_alert_async(f"👤 <b>NEW USER REGISTERED:</b> {username} ({email})")
...
def api_forgot_password():
    ...
    if user:
        ...
        conn.commit()
        conn.close()
        send_tg_alert_async(f"🔑 <b>Password Reset:</b> {user['username']}")
...
def get_bot_logs(bot_id):
    ...
    return jsonify({'success': True, 'filename': bot['filename'], 'logs': 'No logs generated yet.'})
...
def bot_action_api(bot_id, action):
    ...
    if action == 'start':
        ...
        if os.path.exists(file_path):
            ...
            send_tg_alert_async(f"🚀 <b>Bot Launched 24x7:</b> {bot['filename']} by {user['username']}")
...
def buy_plan_api():
    ...
    # 📲 Instant Telegram Notification with Buttons
    ...
    send_tg_alert_async(caption_text, photo_saved_path, tg_markup)

    return jsonify({'success': True, 'request_id': req_id, 'msg': f"✅ Payment request #{req_id} submitted for verification."})

# ⚡ LIVE 1-CLICK ATOMIC APPROVE / REJECT API (100% RELIABLE)
...
def api_admin_payment_action(pay_id, action):
    ...
    if action == 'approve':
        ...
        """, (txn_code, int(pay['user_id']), str(pay['username']), float(pay['amount']), coins_to_give, f"{pay['item_type']} (#{pay['request_id']})", admin_name, now_time))
        
        conn.commit()
        record_audit(admin_name, "Approved Payment", pay['request_id'], f"Credited ₹{pay['amount']} to {pay['username']}")
        conn.close()

        send_tg_alert_async(f"✅ <b>ORDER #{pay['request_id']} APPROVED!</b>\n👤 Client: {pay['username']}\n💵 Amount: ₹{pay['amount']}")
        ...
        send_tg_alert_async(f"❌ <b>ORDER #{pay['request_id']} REJECTED</b>\n👤 Client: {pay['username']}")
