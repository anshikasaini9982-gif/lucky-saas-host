...
def handle_tg_callbacks(call):
    ...
    if data.startswith("tg_pay_appr_") or data.startswith("tg_pay_rejc_") or data.startswith("tg_appr_") or data.startswith("tg_rejc_"):
        ...
        if action == "approve":
            ...
            """, (txn_code, int(pay['user_id']), str(pay['username']), float(pay['amount']), coins_to_give, str(pay['item_type']), str(pay['request_id']), now_time))
...
def api_admin_payment_action(pay_id, action):
    ...
    if action == 'approve':
        ...
        """, (txn_code, int(pay['user_id']), str(pay['username']), float(pay['amount']), coins_to_give, str(pay['item_type']), str(pay['request_id']), admin_name, now_time))
